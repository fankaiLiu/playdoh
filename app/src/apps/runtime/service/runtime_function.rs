use db::runtime::models::sys_function_dev::*;
use db::runtime::entities::sys_function_dev::*;

use sqlx::{Postgres, Pool};
use uuid::Uuid;
use crate::{Result, pagination::PageTurnResponse};
...
impl RuntimeFuctionService{
    ...
    pub async fn add_function_dev(db: &Pool<Postgres>, req: AddReq) -> Result<String> {
    ...
    pub async fn update_function_dev(db: &Pool<Postgres>, function_dev: &UpdateReq,update_by:&Uuid)->Result<Option<FunctionDev>>{
    ...
    pub async fn delete_function_dev(db: &Pool<Postgres>, function_dev_id: &Uuid) -> Result<bool> {
    ...
    pub async fn page_function_dev(db: &Pool<Postgres>, page: i64, page_size: i64) -> Result<FnDevPageResponse> {
        let res = sqlx::query_as!(
            FunctionDev,
            // language=PostgreSQL
    ...
    pub async fn run(db: &Pool<Postgres>, function_id: &Uuid)->Result<String>
    {
        let record =sqlx::query!("select code from sys_function_dev where function_id=$1",function_id).fetch_one(db).await?;
